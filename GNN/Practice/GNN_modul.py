...
class GCN(torch.nn.Module):
    def __init__(self, num_features, num_classes):
        ...
        self.mlp = nn.Linear(16, num_classes)
...
class GraphSage(nn.Module):
	...
	def forward(self, nodes_batch):
		"""
		Generates embeddings for a batch of nodes.
		nodes_batch	-- batch of nodes to learn the embeddings -> 전체 하나의 그래프에서 일부 노드들에 대해서만 학습하기에 "node batch"라는 개념이 탄생
		"""
		lower_layer_nodes = list(nodes_batch) # 입력된 node_batch를 리스트로 변환
		nodes_batch_layers = [(lower_layer_nodes,)] # node_batch_layers를 준비. 리스트 안에 튜플 형식
